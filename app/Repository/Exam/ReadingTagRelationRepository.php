...
use App\Model\Store\StoreExamReadingTagRelation;
 ...
 * 问答知识点关联
 *
 * Class ExamReadingTagRelationRepository
 * @package App\Repository\Store\Exam
 */
class ReadingTagRelationRepository implements StoreRepositoryInterface
{
    /**
     * @Inject()
     * @var StoreExamReadingTagRelation
     */
    ...
    public function repositoryUpdate(array $updateWhere, array $updateInfo): int
    {
        ...
        $relationInfo = $this->relationModel::query()->where('exam_uuid', '=', $updateWhere['exam_uuid'])->get(['id', 'exam_uuid', 'tag_uuid']);
        ...
        $createArray = [];
        $idArray     = array_column($relationInfo, 'tag_uuid');
        ...
        try {
            if (!empty($deleteArray)) {
                $this->relationModel::query()->where([['exam_uuid', '=', $updateWhere['exam_uuid']]])
                    ->whereIn('tag_uuid', $deleteArray)->delete();
            }
            if (!empty($batchInfo)) {
