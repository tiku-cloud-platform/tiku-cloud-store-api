...
use App\Model\Store\StoreExamReadingCategoryRelation;
 ...
 * 问答分类关联
 *
 * Class ExamReadingCategoryRelationRepository
 * @package App\Repository\Store\Exam
 */
class ReadingCategoryRelationRepository implements StoreRepositoryInterface
{
    /**
     * @Inject()
     * @var StoreExamReadingCategoryRelation
     */
    ...
    public function repositoryUpdate(array $updateWhere, array $updateInfo): int
    {
        ...
        $relationInfo = $this->relationModel::query()->where('exam_uuid', '=', $updateWhere['exam_uuid'])->get(['id', 'exam_uuid', 'category_uuid']);
        ...
        $createArray = [];
        $idArray     = array_column($relationInfo, 'category_uuid');
        ...
        try {
            if (!empty($deleteArray)) {
                $this->relationModel::query()->where([['exam_uuid', '=', $updateWhere['exam_uuid']]])
                    ->whereIn('category_uuid', $deleteArray)->delete();
            }
            if (!empty($batchInfo)) {
