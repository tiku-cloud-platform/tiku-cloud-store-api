...
class MenuRepository implements StoreRepositoryInterface
{
    ...
    {
        if (!empty($this->menuModel::query()->create($insertInfo))) {
            return true;
        }
    ...
    {
        $bean = $this->menuModel::query()
            ->with(['coverFileInfo:uuid,file_url,file_name'])
            ->with(['positionShow:title,value,describe'])
            ->with(['menuType:title,value,describe'])
            ->where($closure)
            ->first($this->menuModel->searchFields);
    ...
    {
        return $this->menuModel::query()->where($updateWhere)->update($updateInfo);
    }
    ...
    {
        return $this->menuModel::query()->where($deleteWhere)->delete();
    }
    ...
    {
        return $this->menuModel::query()->whereIn($field, $deleteWhere)->delete();
    }
}