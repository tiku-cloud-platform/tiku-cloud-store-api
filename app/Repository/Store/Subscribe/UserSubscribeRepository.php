...
namespace App\Repository\Store\Subscribe;

use App\Model\Store\StoreWechatUserSubscribe;
 ...
 */
class UserSubscribeRepository implements StoreRepositoryInterface
{
    /**
     * @Inject()）
    ...
    public function repositorySelect(\Closure $closure, int $perSize): array
    {
        $items = $this->userSubscribeModel::query()
            ->with(["wechatTemplate:title,template_id,uuid,page"])
            ->where($closure)
            ->select($this->userSubscribeModel->searchFields)
            ->orderByDesc('id')
