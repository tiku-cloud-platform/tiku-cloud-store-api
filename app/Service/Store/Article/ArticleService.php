...
class ArticleService implements StoreServiceInterface
{
	...
	public function serviceUpdate(array $requestParams): int
	{
		...
		return $this->articleRepository->repositoryUpdate([
	...
	public function serviceDelete(array $requestParams): int
	{
		...
		return $this->articleRepository->repositoryWhereInDelete($deleteWhere, 'uuid');
	}
	...
	{
		return $this->articleRepository->repositoryFind(self::searchWhere($requestParams));
	}
	...
	public function servicePublish(array $requestParams): array
	{
		...
		if (!empty($accessToken)) {
			if ($this->articleRepository->repositoryWhereInUpdate($updateWhere, 'uuid', ['is_publish' => 1]) == 0) {
				$returnInfo['code'] = 1;
				$returnInfo['msg']  = '发布失败';
			} else {
				$pages = [];
